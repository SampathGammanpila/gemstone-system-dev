...
const Toast: React.FC<ToastProps> = ({ id, message, type, onClose, duration = 5000 }) => {
  ...
  const typeStyles = {
    ...
    error: 'bg-danger-100 text-danger-800 border-danger-500',
  ...
  return (
    <div
      className={`
        rounded-md p-4 shadow-lg border-l-4 flex justify-between items-center max-w-md w-full
    ...
    >
      ...
      <button
        onClick={handleClose}
        className="ml-4 inline-flex text-secondary-400 hover:text-secondary-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500"
        aria-label="Close"
      >
